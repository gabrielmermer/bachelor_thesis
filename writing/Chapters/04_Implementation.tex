...
The next phase of the work had an exploratory meaning. In Design Thinking, this corresponds to the Ideation part of the Double Diamond approach \todo{cite}.
The reason for following that approach instead of going straight into building the finished game is to verify if the game idea is worth executing in the first place. More often than not, it turns out that games that work on paper are not enjoyable or playable in real life due to Expectation confirmation theory \todo{cite}.
...
The usage of natural way of speech, and there's bound to be a lot of ways that people approach navigation. The prototype can be, of course, limited to just a few of those ways, but then what would be the difference between this voice input system and pre-existing solutions such as Talos? \todo{Cite talos}

Not to mention the dent in the immersion that would follow if the natural way of player expression were not supported? It could feel closer to piloting a machine with a set of commands instead of guiding your own body.
To take this argument to the extreme in this prototype.
There is nothing stopping the player from just commanding the game to move to the finish location. 
...
The second idea for testing was a text-based adventure game. \todo{cite}
...
The model was manually quantified to allow to be run on consumer GPU \todo{add quant figure}.
